            ...
            <div className="container">
                <div className="basket_inner">
                    ...
                    < div className="basket-item" style={{ display: cart.length > 0 ? 'block' : 'none' }} >
                        ...
                        <div className="basket_total_l">Количество линеек: <span>{cart.reduce((acc, rec) => acc + rec.amount, 0)}</span></div>
                        <div className="basket_total_t">Количество товаров: <span>{cart.reduce((acc, rec) => acc + rec.amount, 0)}</span></div>
                        <div className="basket_total_t">Стоимость:: <span>{cart.reduce((acc, rec) => acc + rec.price * 5, 0)}</span></div>
                        <div className="basket_total_t">Стоимость:: <span>{cart.reduce((acc, rec) => acc + rec.salePrice, 0)}</span></div>
                    </div>
